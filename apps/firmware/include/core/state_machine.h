#ifndef PAPERHOME_STATE_MACHINE_H
#define PAPERHOME_STATE_MACHINE_H
...
public:
    ...
    /**
     * @brief Get total number of state transitions
     */
    uint32_t getTransitionCount() const { return _transitionCount; }
    ...
    /**
     * @brief Force transition even if validator would block it
     * @param newState The target state
     * @param message Optional message describing the reason
     */
    void forceState(StateEnum newState, const char* message = nullptr) {
        if (newState == _currentState) {
            return;
        }
    ...
    /**
     * @brief Check if NOT in any of the specified states
     * @param states Initializer list of states to check
     */
    bool isNotInAnyState(std::initializer_list<StateEnum> states) const {
        return !isInAnyState(states);
    }
    ...
    /**
     * @brief Operator to get current state
     */
...
#endif // PAPERHOME_STATE_MACHINE_H
