...
void TadoControl::renderZone(Compositor& compositor, int16_t index, int16_t y) {
    ...
    ui::drawSelectionBorder(compositor, MARGIN_X, y, width, ZONE_HEIGHT, isSelected);
    ...
    compositor.drawText(displayName.c_str(), MARGIN_X + 12, y + 28, &FreeSansBold12pt7b, true);

    // Away indicator (top right)
    if (zone.isAway) {
        compositor.drawText("AWAY", MARGIN_X + width - 60, y + 28, &FreeSans9pt7b, true);
    }

    // Current temperature (large, center-left)
    char tempBuffer[32];
    snprintf(tempBuffer, sizeof(tempBuffer), "%.1f", zone.currentTemp);
    compositor.drawText(tempBuffer, MARGIN_X + 12, y + 70, &FreeSansBold18pt7b, true);
    compositor.drawText("C", MARGIN_X + 85, y + 58, &FreeSans12pt7b, true);

    // Target temperature (below current)
    snprintf(tempBuffer, sizeof(tempBuffer), "-> %.1fC", zone.targetTemp);
    compositor.drawText(tempBuffer, MARGIN_X + 12, y + 100, &FreeSans12pt7b, true);

    // Humidity (right side)
    snprintf(tempBuffer, sizeof(tempBuffer), "%.0f%%", zone.humidity);
    compositor.drawText(tempBuffer, MARGIN_X + width - 70, y + 75, &FreeSansBold12pt7b, true);
    compositor.drawText("RH", MARGIN_X + width - 40, y + 75, &FreeSans9pt7b, true);

    // Heating bar (segmented, bottom of zone)
    int16_t barY = y + ZONE_HEIGHT - 28;
    int16_t barWidth = width - 130;
    ui::renderSegmentedBar(compositor, MARGIN_X + 12, barY, barWidth, zone.heatingPower);
    ...
    compositor.drawText(tempBuffer, MARGIN_X + barWidth + 25, barY + 12, &FreeSans9pt7b, true);

    // Connection status
    if (!zone.connected) {
        compositor.drawText("(Offline)", MARGIN_X + width - 90, y + 100, &FreeSans9pt7b, true);
    }
}
